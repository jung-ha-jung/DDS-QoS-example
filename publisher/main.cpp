...
void print_help()
{
  printf("For publisher(counter) node:\n");
  printf("publisher [-h] [-c comment] [-q qos_profile]\n");
  printf("Options:\n");
  printf("\t-h Help           : Print this help function.\n");
  printf("\t-c ${comment}     : Specify the comment. Defaults to `Hello`.\n");
...
int main(int argc, char * argv[])
{
  ...
  int qos_profile = 0;
  cli_option[1] = rcutils_cli_get_option(argv, argv + argc, "-q");
  if (nullptr != cli_option[1])
  {
    qos_profile = atoi(cli_option[1]);
  }

  auto node = std::make_shared<robotis::Counter>(comment, qos_profile);
